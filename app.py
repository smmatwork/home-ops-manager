...
with tab2:
    ...
    with col1:
        ...
        with st.form("add_user_form", clear_on_submit=True):
            ...
            new_diet = st.selectbox("Diet Preference", ["Punjabi Veg", "Non-Veg", "Vegan", "Keto"])
            new_goal = st.text_input("Health Goal", "Weight reduction")
        ...
        # DISPLAY: Fetch and show current users
...
with col2:
    ...
    # Form to add a new helper
    with st.form("add_helper_form", clear_on_submit=True):
        h_name = st.text_input("Name")
        h_role = st.selectbox("Role", ["Maid", "Cook", "Driver", "Gardener", "Nanny"])
        ...
        if st.form_submit_button("Add Helper"):
