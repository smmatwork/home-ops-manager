...
class HomeState(TypedDict):
    ...
    # 4. Limit Status (populated by check_limits node)
    at_user_limit: bool   # True when household has reached max allowed users
    at_helper_limit: bool # True when household has reached max allowed helpers

    # 5. Monthly Message Tracking (populated by check_message_limit node)
    subscription_tier: str   # "free" or "pro"
    monthly_message_count: int  # Messages sent this calendar month
    at_message_limit: bool   # True when monthly message limit is reached
